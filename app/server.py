...
def predict_sentiment_bilstm(text):
    ...
    test_samples = preprocess.process_data([text])
...
def predict_sentiment_naive_bayes(text):
    ...
    processed_text = preprocess.process_data([text])[0]
...
def classify_sentiment(probability):
    ...
    return {
        'sentiment': sentiment,
        'score': min(10, max(1, scaled_score)),
...
def predict():
    try:
        if request.is_json:
            ...
            text = data.get('text', '')
            model_type = data.get('model', 'bilstm').lower()
        else:
...
def get_available_models():
    ...
    if naive_bayes_model is not None and vectorizer is not None:
        available_models.append({
            ...
            'description': 'Traditional machine learning with count vectorizer'
        })
